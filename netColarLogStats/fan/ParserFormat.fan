...
class ParserFormat
{
	...
	** TODO: .* can be a bit slow, maybe use :   [^"\\]*(\\.[^"\\]*)*
	static const Regex NcsaCombined := Regex<|^(\S+)\s+(\S+)\s+(\S+)\s+\[([^\]]+)\]\s+"(\S+)\s+(\S+)\s+(\S+)"\s+(\d+)\s+(\S+)\s+"(.*)"\s+"(.*)"$|>
	** Locale string fro Ncsa date format. Ex: 08/Jun/2010:01:43:17 -0400
